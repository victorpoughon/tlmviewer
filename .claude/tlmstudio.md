...
The `mode` field is dropped from the message envelope. tlmserver becomes a pure relay: it receives a POST from Python and immediately broadcasts to all connected WebSocket clients. No storage, no replay, no ring buffer. If no client is connected when a message arrives, the message is dropped. Clients connecting after a Python script has run will see nothing until new data is pushed.

This is a breaking change to `tlmprotocol` (remove `mode` from `Envelope`) and `tlmserver` (remove ring buffer and replay logic).

The `topic` field is kept in the envelope for future use.
...
### 1. Scene Manager
- Always-open panel listing all received scenes in order (topic, timestamp, element count)
...
The Python interface to tlmviewer lives in this monorepo as a first-class workspace at `python/`, published to PyPI as `tlmviewer`. It is a small, focused package with no heavy dependencies (stdlib only, or minimal) — its sole purpose is constructing message envelopes and pushing them to tlmserver over HTTP.

torchlensmaker (separate repo) imports it as a normal pip dependency: `pip install tlmviewer`.
...
CI keeps JS and Python tooling separate — npm workspaces for JS, pytest/pyproject for Python, no shared lockfile or test runner.

For MVP, the Python package exposes `push_scene()`. The `show()` convenience function (with smart defaults, guessing good panels and sampling parameters) is a future addition.

Removing `mode` from the package is covered in refactors.md step 1.
